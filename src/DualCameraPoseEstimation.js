...
const DualCameraPoseEstimation = () => {
  ...
  const setupPoseEstimation = (poseEstimator, videoElement, canvasElement) => {
    ...
    // Manual frame feeding for pose estimation
    const processVideoFrame = async () => {
      if (videoElement.readyState === 4) { // Ensure video is ready
  ...
  useEffect(() => {
    // Set up MediaPipe Pose for both cameras once devices are available
    if (cameras.length >= 2) {
